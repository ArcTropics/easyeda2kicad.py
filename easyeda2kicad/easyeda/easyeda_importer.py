...
def shift_paths_x(paths: str, add_value: float):
    # Process numbers while skipping letters
    tokens = paths.split(" ")
    for i in range(len(tokens)):
        if re.match(r"^-?\d+(\.\d+)?$", tokens[i]):  # Check if the token is a number
            num = float(tokens[i])
            if (i % 2) == 0:  # Modify every second number (index 1, 3, 5,...)
                num += add_value
            tokens[i] = str(num)  # Convert back to string
...
class EasyedaSymbolImporter:
    ...
    def extract_easyeda_data(self, ee_data: dict, ee_data_info: dict) -> EeSymbol:
        ...
        if len(ee_data["dataStr"]["shape"]) == 0:
            ...
            symbol_descriptors.append(ee_data["dataStr"])
